...
export function RunWorkspaceSection() {
  const { rundownId, rundowns, rundown, selectedProduction, disabledCueCount, apiConnection, streamConnection, sessionId, unresolvedOutbox, programmePreviewUrl, runWorkspace } = useAdminState();
    ...
    return (
      <section className="section run-entry" aria-labelledby="run-entry-title">
        ...
        <dl className="run-entry__summary">
          ...
          <div><dt>Rundown</dt><dd>{rundowns.find((i) => i.id === rundownId)?.name ?? "Not selected"}</dd></div>
        ...
        {unresolvedOutbox.length > 0 && <ValidationMessage type="error" message="Resolve or acknowledge the listed dispatch issues before operating live. Retry and Cancel remain available below." />}
        <PrimaryAction className="run-entry__go" disabled={!rundownId || apiConnection !== "connected" || streamConnection !== "connected"} onClick={runWorkspace.enterRun}>
  ...
  const right = (
    <WorkspacePanel heading="Live control" variant="run">
      ...
      <div style={{ marginTop: 14 }}>
        ...
        <p style={{ color: "#fff", fontSize: "1.25rem", fontWeight: 750, marginTop: 4 }}>{runWorkspace.runCue?.label ?? "No enabled cue remaining"}</p>
        <p style={{ color: "#bdc6d4", fontSize: ".86rem", marginTop: 2 }}>{runWorkspace.runCue ? `${runWorkspace.runCue.order}. ${runWorkspace.runCue.status}` : "The rundown is complete or has no enabled cues."}</p>
      </div>
      ...
      <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 10 }}>
        <SecondaryAction disabled={runWorkspace.runCueIndex === 0} onClick={() => runWorkspace.setRunCueIndex((i) => Math.max(0, i - 1))}>Previous</SecondaryAction>
        <PrimaryAction className="action-btn--go" disabled={unresolvedOutbox.length > 0 || !runWorkspace.runCue || !runWorkspace.runCue.enabled || runWorkspace.runCue.status === "active" || runWorkspace.runCue.status === "cancelled"}
          onClick={() => { if (runWorkspace.runCue) runWorkspace.goCue(runWorkspace.runCue); }}>
      ...
      <div style={{ marginTop: 16, borderTop: "1px solid #4c2930", paddingTop: 14, display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <DangerAction onClick={() => runWorkspace.setConfirmation("abandon")}>Abandon session</DangerAction>
        <SecondaryAction onClick={() => runWorkspace.setConfirmation("reset")}>Reset live session</SecondaryAction>
      </div>
    </WorkspacePanel>
  );
...
export function ConfirmationDialog() {
  const { runWorkspace } = useAdminState();
  ...
  return (
    <div className="confirmation-backdrop" role="presentation">
      <section className="confirmation-dialog" role="alertdialog" aria-modal="true" aria-labelledby="confirmation-title" aria-describedby="confirmation-description">
        <h2 id="confirmation-title">{runWorkspace.confirmation === "complete" ? "Complete this live show?" : runWorkspace.confirmation === "abandon" ? "Abandon this live session?" : runWorkspace.confirmation === "safe-clear" ? "Confirm Safe Clear?" : "Start a new live session?"}</h2>
        <p id="confirmation-description">{runWorkspace.confirmation === "complete" ? "The current session will be marked complete. Programme presentation is not cleared automatically." : runWorkspace.confirmation === "abandon" ? "The current session will be recorded as abandoned. Programme presentation is not cleared automatically." : runWorkspace.confirmation === "safe-clear" ? "Safe Clear removes active presentation graphics. Programme video will continue uninterrupted." : "The current live session will be completed and a fresh immutable rundown session will begin."}</p>
        <div>
          <button onClick={() => runWorkspace.setConfirmation(null)}>Cancel</button>
          <button ref={runWorkspace.confirmationButton} className="danger" onClick={runWorkspace.confirmSessionAction}>{runWorkspace.confirmation === "safe-clear" ? "Confirm Safe Clear" : "Confirm"}</button>
        </div>
      </section>
    </div>
  );
