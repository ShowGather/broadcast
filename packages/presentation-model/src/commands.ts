...
export function resolvePresentationCommand(event: PresentationCommandInput, targetPts: number): PresentationCommand[] {
  const { p } = event;
  switch (p.k) {
    case "score": return [{ action: "activate", eventId: `${event.id}:score`, targetPts, region: "video.overlay", layer: "scorebug", priority: 10, item: { kind: "scorebug", homeTeam: "HOME", homeScore: String(p.h), awayTeam: "AWAY", awayScore: String(p.a), clock: p.l } }];
    case "lower": return [{ action: "activate", eventId: `${event.id}:lower`, targetPts, region: "video.overlay", layer: "lower-third", priority: 20, durationMs: p.d ?? 8_000, item: { kind: "lower-third", title: p.t, subtitle: p.s } }];
    case "alert": return [{ action: "activate", eventId: `${event.id}:alert`, targetPts, region: "header", layer: "alert", priority: 100, durationMs: p.d ?? 8_000, item: { kind: "alert", title: p.t, message: p.m, severity: p.x === "c" ? "critical" : p.x === "i" ? "info" : "warning" } }];
